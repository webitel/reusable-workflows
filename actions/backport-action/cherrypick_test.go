...
func TestCreateCherryPickBranch(t *testing.T) {
	...
	t.Run("It should cherry-pick multiple SHAs in order", func(t *testing.T) {
		var (
			testCommitDate, _ = time.Parse(time.RFC3339, "2020-01-02T00:00:00Z")
			branch            = "example"
			shas              = []string{"aaa111", "bbb222"}
			opts              = BackportOpts{
				Target: ghutil.Branch{
					Name: "release-1.0.0",
					SHA:  "fdsa4321",
		...
		expect := []string{
			"git fetch origin aaa111",
			"git fetch origin bbb222",
			"git fetch origin release-1.0.0:refs/remotes/origin/release-1.0.0",
			"git fetch --shallow-since=2020-01-02",
			"git checkout -b example origin/release-1.0.0",
			"git cherry-pick -x aaa111",
			"git cherry-pick -x bbb222",
		}
